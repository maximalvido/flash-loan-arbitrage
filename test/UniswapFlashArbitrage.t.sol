...
contract UniswapFlashArbitrageTest is Test {
    ...
    function test_ExecuteArbitrage_SuccessfulArbitrage() public {
        ...
        routes[0] =
            UniswapFlashArbitrage.SwapRoute({pool: swapPool, zeroForOne: true, tokenIn: token0, tokenOut: token1});

        routes[1] =
            UniswapFlashArbitrage.SwapRoute({pool: swapPool, zeroForOne: false, tokenIn: token1, tokenOut: token0});
